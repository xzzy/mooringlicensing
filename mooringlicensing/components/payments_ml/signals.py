            ...
            for fee_period in instance.fee_season.fee_periods.all():
                for vessel_size_category in instance.vessel_size_category_group.vessel_size_categories.all():
                    for proposal_type in proposal_types:
                        ...
                        if created:
                            pass
                            logger.info('FeeItem created: {} - {} - {}'.format(fee_period.name, vessel_size_category.name, proposal_type.description))

            if instance.num_of_times_used_for_payment == 0:  # Just making sure the fee constructor object has not been used
